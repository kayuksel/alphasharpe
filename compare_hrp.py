...
# Define selection ratios (from 10% up to 100% of assets)
...
for ratio in selection_ratios:
    ...
    # HRP weights for AlphaSharpe selection
    hrp_weights_alpha = hrp_portfolio(train_alpha)
    hrp_returns_alpha = hrp_weights_alpha.dot(test_alpha)
    ...
    results['alpha_hrp'].append(calc_sharpe(hrp_returns_alpha))
...
plt.plot(selection_ratios, results['alpha_equal'], marker='o', label='AlphaSharpe Metric + 1/N')
#plt.plot(selection_ratios, results['alpha_hrp'], marker='^', label='AlphaSharpe + HRP')
